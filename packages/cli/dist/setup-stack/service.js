...
function includesPathEntry(envPath, target) {
    if (!envPath)
        return false;
...
function withoutNpmInjectedBins(env) {
...
async function npmGlobalPrefix(deps) {
...
async function windowsGlobalShimStatus(deps, env) {
    if (deps.platform !== "win32" || !deps.pathExists)
        return undefined;
