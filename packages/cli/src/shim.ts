...
function resolveCompanionBypassPath(configuredOpencodePath: string, resolvedCandidates: string[]): string | null {
  ...
  for (const candidate of resolvedCandidates) {
    ...
    if (resolved.toLowerCase() === configuredLower) continue;
    if (parsed.name.toLowerCase() !== "opencode" || parsed.ext.length > 0) continue;
    if (path.dirname(resolved).toLowerCase() !== configuredDir.toLowerCase()) continue;
    return resolved;
  }
  return null;
}
...
async function installManagedFile(
  ...
  if (await fileExists(backupPath, fsApi)) {
    return {
      ok: false,
      message:
        `Refusing install: backup already exists at '${backupPath}' while '${targetPath}' is not managed by mpp. ` +
...
export async function collectShimDiagnostics(
  ...
  const interceptionByCompanion = Boolean(
    companionBypassPath &&
      resolvedLower &&
...
export async function installOpencodeShim(
  ...
  if (!cmdInstall.ok) return { ok: false, message: cmdInstall.message };

  if (companionBypassPath && companionBackupPath) {
    const companionInstall = await installManagedFile(
      companionBypassPath,
      companionBackupPath,
      buildWindowsCompanionScript(),
      fsApi
    );
    if (!companionInstall.ok) return { ok: false, message: companionInstall.message };
  }
  ...
  if (!diagnostics.launcherInterceptionOk) {
    return {
      ok: false,
      message:
        `${diagnostics.launcherInterceptionReason} Managed path: '${diagnostics.configuredOpencodePath}'. ` +
  ...
  if (await fileExists(companionBackupPath, fsApi)) {
    ...
    if (currentCompanion.length > 0 && !currentCompanion.includes(SHIM_MARKER)) {
      return {
        ok: false,
        message:
          `Refusing uninstall: '${companionPath}' is not mpp-managed while '${companionBackupPath}' exists. ` +
