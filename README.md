...
Install the plugin directly inside OpenCode:

```bash
opencode plugin -g multi-profile-provider-opencode-plugin@latest
```

Then from OpenCode's command palette, create and select a profile:
...
Close OpenCode and relaunch it through MPP to apply isolation:
...
Create profiles, select one, and launch:
